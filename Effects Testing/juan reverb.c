...
int main(void) {
	...
	while(var != END_INDEX) {
		...
		while(1) {
			...
			int length = 3000 * 0.2;
			Buffer1[ length ];
			int readLength = 0, writeLength = length - 1;

			float delay.readNext (float x = 2 ) {
				float ret = Buffer1[readLength];
				++readLength;
			...
			void delay.writeNext (float x = 2) {
				Buffer1 [writeLength] = x;
				++writeLength;
				if ( writeLength >= length)
					writeLength = 0;
			}

			float echo (float x = 2, float g = 0.4 ) {
				float ret = x + g * delay.readNext( x );
				delay.writeNext( ret );
				return ret;
			}
