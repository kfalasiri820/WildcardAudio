...
void mainEffectsAndLoop(volatile Uint32 in[256], volatile Uint16 out[256]){
	for(int i = 0; i < BUFFER_SIZE; i++){//Batch process through 256 samples
		...
		/*********************************DELAY*****************************/
		//if(delayOn){//add delayed signal with live signal
			//note, change these gains to something lower when using loops
			Uint16 delayAndLive = (Uint16)(0.5 * (float)live + (Uint16)(0.5 * (float) inputBuffer[delayBufferPrev]));
			out[i] = delayAndLive;//put to the output
			inputBuffer[delayBufferCurr] = delayAndLive;//save to the input buffer to use for feedback

			//Increment pointers
			delayBufferCurr++;
			delayBufferPrev++;
			if(delayBufferCurr >= INPUT_BUFFER_SIZE)//if out of bounds
				delayBufferCurr = 0;//reset to 0
			if(delayBufferPrev >= INPUT_BUFFER_SIZE)//if out of bounds
