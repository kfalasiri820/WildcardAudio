...
Uint32 loop1Index = 0;			//Index used to traverse through loop1
...
Uint32 loop2Index = 0;			//Index used to traverse through loop1
...
__interrupt void loop2Click_ISR(void){
    ...
    recording2 ^= 1;
    PieCtrlRegs.PIEACK.all = PIEACK_GROUP2; //acknowledges the int
    EINT;
}
		...
		while(!ready){//Batch process while waiting for DMA to finish
			...
			if(program_state == 0){
                for(int i = 0; i < BUFFER_SIZE; i++){//Batch process through 256 samples
						...
						loop1Index = 0;//Start the loop1Index at 0
						loop1Clicks++;//Increment the number of clicks
					}
                		if(loop1Clicks == 3){//If you pressed the loop 1 twice (clicks = 3)
						loopLength = loop1Index;//mark the loop length
						loop1Clicks++;
					}
                		...
                		ping_2[i] = (Uint16)(0.5 * (float) live);//Always output the live signal

                		//if you aren't muted, add the loop to the live signal
					//if(!muteLoop)
						ping_2[i] += (Uint16)(0.5 * (float)loop1[loop1Index]);

					//if you are recording1, OVERWRITE the live to the loop
					if (recording1){
						loop1[loop1Index] = (Uint16)(0.5 * (float) live) +(Uint16)(0.5 * (float)loop1[loop1Index]) ; //+ 0.5 * loop1[loop1Index];
						loop1Index++;
					}
					else if (recording2){
						loop2[loop2Index] = (Uint16)(0.5 * (float) live) +(Uint16)(0.5 * (float)loop2[loop2Index]) ;
						loop2Index++;
					}
					...
					if (loop2Index >= loopLength){
						loop2Index = 0;
					}


					//Increment loop1Index and check for overflow
                ...
                for(int i = 0; i < BUFFER_SIZE; i++){
					...
					////////////////////Loop/Overdub implementation////////////////////
					//NOTE: ping_1 is the input, ping_2 is the output
					live = (Uint16)((pong_1[i] >> 2) & 0xFFFF);//define the live signal

					pong_2[i] = (Uint16)(0.5 * (float) live);//Always output the live signal
